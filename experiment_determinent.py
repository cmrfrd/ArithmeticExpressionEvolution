...
from fitness import fitness_determinent
...
MUTATE = [
    ...
    ("update_weight", 0.1),
    ("update_head_edges", 0.6),
]

experiment_name = "approximate_determinent"
run_id = make_name()
...
write_chunks = 16
agents = list(init_population(out_dim, population_size, init_num_mutations, NODES, MUTATE, env=None))
...
for gen in (gen_bar := tqdm.tqdm(range(num_generations), leave=None)):
    ...
    for agent in agents:
        fitness = fitness_determinent(agent)
    ...
    new_agents_results = pool.starmap(
        p_mutate_new_agent,
        map(lambda item: (serialize(item[0]), item[1], "matrix", str(S_prefix), db_filename), agent_and_num_mutations)
    )
