...
from fitness import fitness_pi
...
MUTATE = [
    ...
    # ("update_random_node_type",0.1),
    ("update_head_edges", 0.3),
]

experiment_name = "approximate_pi"
run_id = make_name()
...
num_generations = 16
mutation_range, mutation_mush_perc = (0,4), 0.5
...
for gen in (gen_bar := tqdm.tqdm(range(num_generations), leave=None)):
    ...
    new_agents_results = pool.starmap(
        p_mutate_new_agent,
        map(lambda item: (serialize(item[0]), item[1], "pi", str(S_prefix), db_filename), agent_and_num_mutations)
    )
